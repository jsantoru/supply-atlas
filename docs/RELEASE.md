...
The defining flow is product → documented part → supplier or manufacturing facility → individually scoped evidence → another product sharing that exact part. Product studio includes twelve generated photorealistic illustrations. Nine electronics products use component selection and layer separation; Mohajer-6 and Shahed-238 use conceptual major-system cards and cited historical dossiers. LUCAS adds an interactive exterior diagram with moving exterior sections, synchronized source inspection and a default documented network covering industrial attribution, funding, evaluation and program context. Its Supplier claims tab retains the manufacturing graph. The graph, component hierarchy, clustered map, profiles, global search, shareable URL filters, product/supplier comparison and hypothetical disruption views use the same scoped manufacturing-claim model.

Aircraft dossiers contain 38 additional sources, 14 major-system topics and 81 dated narrative entries plus 15 separate network records spanning chronology, organizations, public leadership, manufacturing context and evidence limits. Dossier narratives cover all cited dates; workspace filters apply only to the separately labeled manufacturing attributions. Generated exteriors are not CAD models or engineering teardowns. No detailed weapons parts lists, operational guidance or military-site coordinates are included. See [DRONE-RESEARCH.md](DRONE-RESEARCH.md).
...
84 entities: 12 products, 24 parts/configurations, 17 companies, 3 facilities, 2 materials, plus categories, industries, regions and variants. The collection contains 38 scoped claims and 55 sources, including the 38 aircraft research references. Narrative system categories and leadership records are not counted as physical components or suppliers.
...
- LUCAS records explicit SpektreWorks developer/manufacturer attributions; 15 separately labeled industrial, evaluation and program relationships across 12 nodes never create component dependencies. See [LUCAS-RESEARCH.md](LUCAS-RESEARCH.md).
...
- 56 backend tests passed, including factory/product isolation, variant/date scope, inference anchors, disputed status, material paths, auth, transactional imports, alias resolution, caching, review persistence, safe upgrades, narrative citation integrity, contextual-record isolation and program-network reference validation.
- 38 browser tests passed across desktop and mobile, including all twelve product studios, the defining evidence flow, URL navigation, keyboard controls, axe accessibility, map selection, supplier comparison, administrative review, research citations, actual LUCAS exterior movement, assembly/reset, image fallback, independent network-request recovery and filter boundaries.
...
- [LUCAS research / issue #12](https://github.com/jsantoru/supply-atlas/issues/12): cited LUCAS dossier, exterior interaction and industrial/program network.
- [LUCAS network depth / issue #14](https://github.com/jsantoru/supply-atlas/issues/14): primary-source funding and institutional links, a prominent documented network, contextual path navigation and separate supplier claims.
