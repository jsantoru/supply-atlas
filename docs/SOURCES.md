...
Research accessed September 10, 2026 UTC (September 9 in New York). The collection contains **9 products, 24 specific parts/configurations, 14 companies, 3 facilities, 2 materials, 33 claims and 17 sources**. It covers seven Raspberry Pi computer/microcontroller products, Apple Mac Pro (2019), and Samsung Galaxy S9. Three TSMC part-level process dependencies connect the Raspberry Pi products. These are historical observations, not a live sourcing feed or complete BOM. Unknown memory vendors, chip fabs, packaging sites, material suppliers and alternate sourcing are explicit coverage gaps.
...
Run `python backend/research/build_collection.py` to deterministically regenerate `collection.json` from the reviewed factual records. This command makes no network requests. Startup imports new curated records and updates unchanged curated records; administrator corrections are preserved and seed conflicts are logged for review. The protected review workflow can import a separately reviewed bundle with its reason and evidence.
